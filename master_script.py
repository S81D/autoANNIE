# master script for automated event building runs
...
# Until the new PhaseIITreeMaker is complete
'''
...
while complete_BC != len(BC_resubs):
    ...
    for i in range(len(BC_check)):
        ...
        if BC_check[i] == True and BC_resubs[i] == 0:
            ...
            present = helper_script.check_root(runs_to_run[i],n_jobs,'scratch', output_path, beamcluster_path)
        ...
        # actually complete, transfer
            ...
            present = helper_script.check_root(runs_to_run[i],n_jobs,'persistent', output_path, beamcluster_path)
            if present == False:
...
#os.chdir('../')       # return to input_location from BeamCluster/
time.sleep(1)
...
#os.system('rm BeamCluster/Processed*.tar.gz')    # BC job tar ball of processed files
time.sleep(1)
print('\nExiting...\n')
